...
def run_test_largest_negative_number():
    ...
    # Test 1:
    expected = -12
    answer = largest_negative_number(([3], [-1245], [-12]))
    print('Expected and actual are:', expected, answer)

    # Test 2:
    expected = -2
    answer = largest_negative_number(([1, -2], [2478], [-3]))
    print('Expected and actual are:', expected, answer)
...
def first_is_elsewhere_too(seq_seq):
    ...
    for k in range(1, len(seq_seq)):
        for j in range(len(seq_seq[0])):
            for x in range(len(seq_seq[k])):
                if seq_seq[0][j] == seq_seq[k][x]:
                    return True
    return False
    # ------------------------------------------------------------------
